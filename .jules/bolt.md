...
## 2024-05-18 - [List Removal Optimization]
**Learning:** Calling `.remove()` on a list inside a loop (like in resource managers or connection pools such as `BrowserPool`) causes O(N^2) time complexity because list removal is O(N). This can block the async event loop significantly if the pool size is large.
**Action:** Use a single-pass filter to build a list of retained items and reassign it in-place using slice assignment (`my_list[:] = retained`) to avoid breaking external references.
