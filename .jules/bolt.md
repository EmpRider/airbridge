...
## 2024-05-15 - [Fast-path UTF-8 Validation]
**Learning:** Always applying expensive encode/decode operations to sanitize strings (e.g., removing unpaired UTF-16 surrogates) creates unnecessary overhead, as the vast majority of strings are already valid UTF-8.
**Action:** Implement a fast-path validation check (`try: text.encode('utf-8')`) before falling back to the expensive encode/decode replacement step. This avoids processing overhead for clean strings and yields significant performance gains.
