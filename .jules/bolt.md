...
## 2025-02-15 - [Batched Playwright Locators with or_()]
**Learning:** When checking multiple fallback UI selectors in Playwright, joining selectors with commas (e.g., `", ".join(selectors)`) is brittle, as a single invalid selector (like a malformed XPath or CSS rule) will cause the entire query to fail. A safer way to batch locators is to use `locator.or_()` programmatically, wrapping the combined query in a tight `try...except` block, and falling back to sequential evaluation if the combined batch fails.
**Action:** When evaluating multiple fallback selectors in Playwright, prefer combining them using `locator.or_()` inside a `try...except` block. If `or_()` fails (e.g. due to invalid selectors), fall back to looping over them sequentially, ensuring fault tolerance and avoiding N+1 query patterns.
