...
## 2025-02-12 - Optimize extremum finding with min()
**Learning:** Sorting an entire list to find a single extremum (like the oldest item) incurs unnecessary O(N log N) overhead.
**Action:** Use the built-in `min()` or `max()` functions with a `key` argument for O(N) complexity when finding a single extremum.
