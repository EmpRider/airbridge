...
## 2024-05-18 - [UI State Verification Before Action]
**Learning:** In Playwright UI automation, blindly executing click operations to open menus and waiting for their visibility can be a costly operation in terms of execution time and network latency, especially when the application is already in the desired state.
**Action:** Optimize performance by verifying if the desired state is already active (e.g., inspecting a dropdown trigger's `inner_text`) before unconditionally executing expensive operations like clicking to open menus and waiting for visibility. When checking if a specific model (e.g., 'GPT-4') is active, use robust regex matching with negative lookbehinds/lookaheads (e.g., `re.search(r'(?<![\w\-])' + re.escape(model_name) + r'(?![\w\-])', text)`) instead of simple substring matching (`in`) to prevent false positives with overlapping names like 'GPT-4' and 'GPT-4-turbo'.
