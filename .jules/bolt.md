...
## 2024-08-03 - [O(N) vs O(N log N) for finding oldest idle context]
**Learning:** When finding a single extremum (like the oldest context to evict in a browser pool), sorting the entire list with `.sort()` incurs unnecessary O(N log N) overhead, which can be inefficient as the pool scales.
**Action:** Use the built-in `min(iterable, key=...)` function, which performs a single O(N) pass to find the extremum, instead of fully sorting the list.
