...
## 2024-05-18 - [Optimizing UI State Checks Before Actions]
**Learning:** In Playwright UI automation, blindly executing expensive operations like waiting for visibility and clicking to open a dropdown menu (e.g., `await btn.wait_for(...)` and `await btn.click()`) when the desired state might already be active causes unnecessary delays and DOM interactions.
**Action:** Always attempt to read the current state first using fast, non-blocking attribute or text checks (e.g., `await btn.get_attribute("aria-pressed", timeout=1000)` or `await btn.inner_text(timeout=1000)`). If the desired state is already active, return early to skip the expensive UI operations. Use robust regex matching (`re.search(r'(?<![\w\-])' + re.escape(model_name) + r'(?![\w\-])', text)`) to avoid false positives with overlapping text names.
