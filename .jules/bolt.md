...
## 2024-05-20 - [List Removal Inside Loop]
**Learning:** Calling `list.remove()` inside a loop that iterates over a subset of the list to remove items is an O(N^2) operation, because each `remove()` call scans the list linearly. When performed inside an asyncio lock, this can lead to severe CPU spikes and lock contention during large resource cleanups.
**Action:** Replace `list.remove()` inside loops with an O(N) single-pass list comprehension or loop that builds a list of retained items, and reassign it to the original list using slice assignment (`my_list[:] = retained`) to preserve external references while ensuring O(N) performance.
