...
## 2025-02-12 - [Playwright Unconditional UI Interaction Latency]
**Learning:** In Playwright UI automation, unconditionally executing expensive operations (e.g., clicking to open a dropdown menu, waiting for it to become visible, and extracting its items) adds significant latency per execution (~1-2 seconds per turn). This is especially costly when the desired state (e.g., the correct model being selected) is already active.
**Action:** Always optimize performance by verifying if the desired state is already active (e.g., inspecting a dropdown trigger's `inner_text`) before unconditionally executing expensive DOM interactions.
