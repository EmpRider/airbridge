...
## 2025-02-15 - [Playwright Concurrent Locator Count for Fallbacks]
**Learning:** Evaluating multiple fallback selectors sequentially via `await page.locator(sel).count()` in a loop creates an N+1 network latency bottleneck. However, batching them into a single string `", ".join(selectors)` can break syntax or fault isolation if any selector inherently contains commas or is malformed.
**Action:** Use `asyncio.gather(*(page.locator(sel).count() for sel in selectors), return_exceptions=True)` to evaluate multiple fallback selectors concurrently, ensuring fault isolation and reducing latency from O(N) to O(1) network round-trips.
