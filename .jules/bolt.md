...
## 2024-05-18 - [Optimized UI state checks for Select Mode]
**Learning:** In UI automation driven by generic configurations, unconditionally clicking dropdowns and waiting for menus adds significant cumulative latency (up to 25s per turn), especially if the desired state is already active.
**Action:** Always verify current UI state (e.g., using `inner_text`) before executing expensive interaction sequences. Ensure that generic substring matching checks (`in`) are replaced with robust regex boundary checks (e.g., `(?<![\w\-])` and `(?![\w\-])`) to prevent false positive matches on similarly named states (like 'GPT-4' vs 'GPT-4-turbo').
