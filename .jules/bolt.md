...
## 2024-05-18 - Checking dropdown trigger text before clicking in Playwright
**Learning:** In Playwright automation, unconditionally opening dropdown menus causes unnecessary network round-trips, DOM interaction overhead, and explicit sleeps when the target state might already be met.
**Action:** Always check the current UI state (e.g., examining the dropdown trigger button's `inner_text`) before performing expensive state-changing interactions to avoid unnecessary latency.
