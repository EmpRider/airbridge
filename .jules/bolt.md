...
## 2025-02-15 - [O(N) Extrema Selection over O(N log N) Sorting]
**Learning:** When finding the oldest or smallest element in an array (e.g., when evicting a resource), using `.sort()` adds unnecessary O(N log N) overhead, which can become noticeable as the list size grows.
**Action:** Use the built-in `min(iterable, key=...)` or `max()` function which operates in O(N) complexity to find a single extremum without sorting the entire list.
