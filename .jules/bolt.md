...
## 2025-02-14 - [String Surrogates Serialization Performance]
**Learning:** Functions that frequently sanitize or clean text by chaining encode/decode operations with custom error handlers (like `errors="surrogatepass"` and `errors="replace"`) introduce significant overhead. When the vast majority of processed strings are already clean (e.g., standard Playwright text scraping), running every string through the slow replacement path unnecessarily degrades performance.
**Action:** When writing text sanitization functions for edge cases like unpaired UTF-16 surrogates, always implement a fast-path validation check (e.g., `try: text.encode('utf-8')`). This allows clean strings to bypass the expensive replacement logic, dramatically improving performance for the common case without sacrificing robustness.
