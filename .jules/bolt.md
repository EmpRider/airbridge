...
## 2024-05-13 - [Playwright UI State Verification]
**Learning:** Unconditionally performing expensive UI operations—such as clicking a menu trigger to open a dropdown and waiting for its items to become visible—adds significant latency when the desired state is already active. This is especially true for session-based adapters using persistent browser profiles, where the selected model is often remembered from the previous session.
**Action:** Always optimize Playwright UI automation by first checking if the desired state is already active. For instance, inspect the `inner_text` of a dropdown's trigger element before deciding whether to execute the expensive click and wait operations.
