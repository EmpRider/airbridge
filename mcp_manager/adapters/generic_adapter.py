...
class GenericAdapter:
    ...
    async def _needs_login(self, page) -> bool:
        ...
        # OPTIMIZATION: Evaluate fallback selectors concurrently to prevent N+1 network roundtrips
        # while avoiding syntax risks of comma-separated string batching.
        counts = await asyncio.gather(
            *(page.locator(sel).count() for sel in selectors),
            return_exceptions=True
        )

        for sel, count in zip(selectors, counts):
