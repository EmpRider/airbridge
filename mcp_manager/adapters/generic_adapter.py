...
class GenericAdapter:
    ...
    async def enable_temp_chat(self, page):
        ...
        if not selectors:
            ...
            return
        try:
            btn = self._build_fallback_locator(page, selectors).first
    ...
    def _build_fallback_locator(self, page, selectors):
        if not selectors:
            return None
    ...
    async def _needs_login(self, page) -> bool:
        ...
        if not selectors:
            return False
        loc = self._build_fallback_locator(page, selectors)
        try:
    ...
    async def _wait_for_input(self, page):
        ...
        if not selectors:
            return None
        try:
            field = self._build_fallback_locator(page, selectors).first
    ...
    async def _select_mode(self, page, model_name):
        ...
        try:
            ...
            try:
                picker_btn = self._build_fallback_locator(page, picker_selectors).first
                ...
                items_loc = self._build_fallback_locator(page, item_selectors)
