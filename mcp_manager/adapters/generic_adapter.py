...
class GenericAdapter:
    ...
    def _combine_locators(self, page, selectors):
        """
        ⚡ Bolt: Optimize selector evaluation by combining them programmatically
        with locator.or_(). This prevents N+1 network queries when looping,
        and is safer than string joining (which breaks with mixed engine types).
        """
        if not selectors:
            return None
    ...
    async def enable_temp_chat(self, page):
        ...
        if not selectors:
            ...
            return
        try:
            btn = self._combine_locators(page, selectors).first
    ...
    async def _needs_login(self, page) -> bool:
        ...
        if not selectors:
            return False
        try:
            combined = self._combine_locators(page, selectors)
    ...
    async def _wait_for_input(self, page):
        ...
        if not selectors:
            return None
        try:
            field = self._combine_locators(page, selectors).first
    ...
    async def _select_mode(self, page, model_name):
        ...
        try:
            ...
            try:
                picker_btn = self._combine_locators(page, picker_selectors).first
                ...
                try:
                    ...
                    logger.debug(f"Could not read mode picker text, proceeding with click: {e}")

                await picker_btn.click()
                logger.debug("Mode picker clicked successfully")

                items = self._combine_locators(page, item_selectors)
            ...
            try:
                ...
                # This significantly reduces latency when there are many mode items.
