    ...
    async def _select_mode(self, page, model_name):
        ...
        try:
            ...
            try:
                ...
                # OPTIMIZATION: Check if the desired mode is already selected before clicking the dropdown.
                # This avoids expensive DOM mutations and waiting for visibility.
                current_text = await picker_btn.inner_text()
                if current_text and re.search(r'(?<![\w\-])' + re.escape(model_name) + r'(?![\w\-])', current_text):
                    logger.info(f"Mode '{model_name}' is already active. Skipping dropdown interaction.")
                    return
