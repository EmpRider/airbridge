    ...
    async def _select_mode(self, page, model_name):
        ...
        try:
            ...
            try:
                ...
                # ⚡ Bolt: Check if the desired mode is already active before unconditionally
                # opening the dropdown and waiting. This saves significant time (up to 25s timeouts).
                # We use regex with word boundaries/lookarounds to avoid false positives (e.g. GPT-4 vs GPT-4-turbo).
                try:
                    ...
                    logger.debug(f"Could not verify current mode text: {e}")
