    ...
    async def _select_mode(self, page, model_name):
        ...
        try:
            ...
            try:
                ...
                # Check if the mode is already selected to avoid unnecessary UI interactions
                current_text = await picker_btn.inner_text()
                if current_text and re.search(r'(?<![\w\-])' + re.escape(model_name) + r'(?![\w\-])', current_text):
                    logger.info(f"Mode '{model_name}' is already selected (current text: '{current_text.strip()}'). Skipping dropdown.")
                    return
