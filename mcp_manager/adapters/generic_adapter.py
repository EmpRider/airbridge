    ...
    async def _select_mode(self, page, model_name):
        ...
        try:
            ...
            try:
                ...
                # OPTIMIZATION: Early return if model is already selected to save network trips and wait time
                try:
                    current_text = await picker_btn.inner_text()
                    if re.search(r'(?<![\w\-])' + re.escape(model_name) + r'(?![\w\-])', current_text):
                        logger.info(f"Mode '{model_name}' is already selected, skipping picker interaction.")
                        return
                except Exception as inner_e:
                    logger.debug(f"Could not read current picker text: {inner_e}")
            ...
            try:
                ...
                for i, text in enumerate(all_texts):
                    ...
                    # OPTIMIZATION: Use robust regex matching instead of basic 'in' to prevent overlapping false positives
